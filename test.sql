...
CREATE TABLE IF NOT EXISTS `offices` (
  ...
  -- указание длинны для числового типа, по моему мнению, бессмысленное мероприятие, 
  -- так это не влияет ни на размер ни на максимальное число.
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  -- тут у меня та же ремарка про указание длинны числового типа
  `name` varchar(50) COLLATE utf8_unicode_ci DEFAULT NULL COMMENT 'ФИО пользователя',
  -- согласно коментарию - тут мы храним ФИО, а опираясь на название поля - храним только Имя,
  -- для храниния Имени - длины в 50 символов, вероятно хватит, но для храниения такого ФИО как
  -- Константин Константинович Константинопольский-Олтай-Оглы - длинны не хватит, по этому:
  -- либо увеличить длинну до varchar(100), либо добавить еще 2 поля, для Фамилии и Отчества.
  `office_id` smallint(6) DEFAULT NULL COMMENT 'offices.id - ID офиса',
  -- в таблице 'offices' мы используем для ID - TINYINT, а тут почему-то SMALLINT
  PRIMARY KEY (`id`),
